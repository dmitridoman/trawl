...
- **Lighthouse** — Performance, Accessibility, Best Practices, SEO scores, **plus** the millisecond metrics (LCP, FCP, TBT, CLS, Speed Index, TTI, TTFB), perf **opportunities** with estimated ms/byte savings and the specific offending resources, **diagnostics** (the actual LCP element, layout-shift elements, third-party breakdown, main-thread work), and every other failing audit with Lighthouse's own remediation text — all embedded inline. Runs via **PageSpeed Insights** (no local Chrome, no local-machine load skewing scores) when `CRAWLSHOT_PSI_KEY` is set (free at [Google Cloud Console](https://console.cloud.google.com/apis/library/pagespeedonline.googleapis.com)) and the crawl is unauthenticated; otherwise (or on a per-page PSI failure) it falls back to a real local Lighthouse run. Full per-page report is still linked — a local HTML file, or the live pagespeed.web.dev page for PSI.
...
- **Search volume / CPC / competition** — average monthly search demand for the same `--rank` keywords, via **Keywords Everywhere**. Set `CRAWLSHOT_KEYWORDS_EVERYWHERE_KEY` ([keywordseverywhere.com](https://keywordseverywhere.com/)). Works independently of the Brave key — set either or both.
